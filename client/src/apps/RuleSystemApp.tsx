...
/* ── SVG 网络图 ──────────────────────────────────────────────────────── */
function RuleNetwork({ nodes, edges, posMap, ruleStates, groupColorMap, selectedId, onSelect }: { nodes: any[]; edges: any[]; posMap: any; ruleStates: any; groupColorMap: Record<string, string>; selectedId: any; onSelect: any }) {
  return (
...
function RuleTriggerEditor({ draft, setDraft, statDefs, events, editingId, isNew, onSave, onClose }: { draft: any; setDraft: any; statDefs: any[]; events: any[]; editingId: any; isNew: boolean; onSave: any; onClose: any }) {
  const upd = (k, v) => setDraft(p => ({ ...p, [k]: v }));
  const existingGroups: string[] = [...new Set(events.map(e => e.eventConfig?.exclusionGroup).filter(Boolean))];
