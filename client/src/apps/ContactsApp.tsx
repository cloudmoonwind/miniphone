...
// --- 组件：联系人 (结缘) App ---
const DEFAULT_FORM: { id?: string; name: string; avatar: string; tags: any[]; group: string; core: string; persona: string; sample: string; timezone: string; apiPresetId: any; isFavorite: boolean; isBlacklisted: boolean } = { name:'', avatar:'', tags:[], group:'', core:'', persona:'', sample:'', timezone:'', apiPresetId:null, isFavorite:false, isBlacklisted:false };
...
const ContactsApp = ({ onBack, onStartChat }) => {
  ...
  const grouped = useMemo(() => visible.reduce<Record<string, any[]>>((acc,c)=>{ const g=c.group||'未分组'; (acc[g]||(acc[g]=[])).push(c); return acc; }, {}), [visible]);
