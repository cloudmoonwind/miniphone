...
export function AppProvider({ children, onNavigate }) {
  const [activeChar, _setActiveChar] = useState(null);
  const [activePreset, setActivePreset] = useState(null);
  const [wallpaper, setWallpaperState] = useState(
    () => localStorage.getItem('ics_wallpaper') || null
  );
  const [recentChat, setRecentChatState] = useState(() => {
  ...
  const setActiveChar = useCallback((char) => {
    _setActiveChar(char);
    eventBus.emit('char:activated', { char });
  }, []);

  const updateWallpaper = useCallback((url) => {
    setWallpaperState(url);
    try { localStorage.setItem('ics_wallpaper', url || ''); } catch {}
  }, []);

  const updateRecentChat = useCallback((char, content) => {
  ...
  const navigate = useCallback((appId, params = {}) => {
