...
| 阶段 | 主题 | 状态 |
|------|------|------|
| 阶段 1 | 变量管道底座 + 占位符全链路 + 能力清单页 | ✅ 完成 |
| 阶段 2 | AI 输出协议统一（`<sys>` 块） | ✅ 完成（含插队修复 `branch:<outcome>` hack） |
| 阶段 3 | ConversationMode 条目式实现 | ✅ 完成 |
| 阶段 4 | Trigger/Effect 准抽出（注册表化） | ✅ 完成 |
| 阶段 5 | 预设参数化（`preset:` 命名空间） | ⏸️ 暂缓 — 当前用户既是作者又是消费者，参数化层不需要 |
| 阶段 6 | 后续：真正抽离 / 后处理规则 / 记忆系统 / promptStore 改名 | 长期方向 |

阶段 1-4 的细节已沉淀到：
- [ICS_AI输出协议_v1.md](../../ICS_AI输出协议_v1.md) — 阶段 2 协议契约
- [docs/设计决策记录.md](../设计决策记录.md) — 各阶段关键决策（命名空间表、resolver 同步约束、解析/应用分层、mode 不建表、effect 注册表中英别名等）
- 各 smoke test / self-test（占位符 / AI 协议 / 命名空间 / 上下文组装 / mode 切换 / effect 注册表 / trigger 总线）

未来若启动阶段 5，请起新的计划文档而不是复活本文。本文留作历史档案。
